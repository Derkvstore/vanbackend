...
router.put('/:id/cancel', async (req, res) => {
    ...
    try {
        ...
        const updateInvoiceResult = await clientDb.query(
            ...
            [raison_annulation, parseFloat(montant_paye_facture || 0), id] // Use parseFloat(montant_paye_facture || 0) for reimbursement
        );
...
"
J'ai bien noté votre problème persistant avec l'erreur lors de l'annulation de la facture.

L'erreur "could not determine data type of parameter $1" est très spécifique et indique que la base de données ne sait pas comment interpréter une valeur que vous lui envoyez. Dans le code que vous avez sélectionné, le problème se trouve très probablement dans la ligne suivante de la route `PUT /api/factures/:id/cancel`:

```javascript
            [raison_annulation, montant_paye_facture, id] // Use montant_paye_facture for reimbursement
```

Ici, `montant_paye_facture` est passé en tant que paramètre `$2` pour la colonne `montant_rembourse`. Si `montant_paye_facture` est `null` ou `undefined` (ce qui peut arriver si la facture n'a jamais eu de paiement enregistré), PostgreSQL ne peut pas déterminer le type de données attendu pour `montant_rembourse`, d'où l'erreur.

Pour résoudre cela, nous devons nous assurer que la valeur passée pour `montant_rembourse` est toujours un nombre, même si elle est initialement nulle.

J'ai mis à jour le Canvas `factures-module` pour corriger ce problème.

**Modification effectuée :**

* Dans la route `PUT /api/factures/:id/cancel`, la valeur de `montant_paye_facture` passée pour le paramètre `$2` est maintenant explicitement convertie en un nombre flottant, avec une valeur par défaut de `0` si elle est nulle ou indéfinie.
    * Ancien: `[raison_annulation, montant_paye_facture, id]`
    * Nouveau: `[raison_annulation, parseFloat(montant_paye_facture || 0), id]`

Veuillez trouver le code mis à jour ci-dessous. Remplacez le contenu de votre fichier `backend/factures.js` par ce nouveau code et **redéployez votre backend sur Render**. Cela devrait résoudre l'erreur de type de données.
